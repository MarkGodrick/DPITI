...
class NNhistogram(Histogram):
    ...
    def __init__(
        ...
        self._priv_data_emb = np.stack(priv_data_emb.data_frame[IMAGE_DATA_COLUMN_NAME].values, axis=0).astype(np.float32)
    ...
    def compute_histogram(self, priv_data, syn_data):
        ...
        if self._priv_data_emb is None:
            execution_logger.info("Histogram: No computed embeddings of private dataset. Start computation...")
            ...
            self._priv_data_emb = priv_embedding
            execution_logger.info("Histogram: Embeddings computation succeed.")
        else:
            execution_logger.info("Histogram: Using computed embeddings of private dataset...")
            priv_embedding = self._priv_data_emb
