...
class _ComputeFID(Callback):
    """The callback that computes the Frechet Inception Distance (FID) between the private and synthetic data."""

    def __init__(self, priv_data, embedding_priv, embedding_syn, filter_criterion=None):
        """Constructor.

        :param priv_data: The private data
        :type priv_data: :py:class:`pe.data.Data`
        :param embedding_priv: The embedding to compute the FID of private data
        :type embedding_priv: :py:class:`pe.embedding.Embedding`
        :param embedding_syn: The embedding to compute the FID of synthetic data
        :type embedding_syn: :py:class:`pe.embedding.Embedding`
        ...
        self._embedding_priv = embedding_priv
        self._embedding_syn = embedding_syn
        self._filter_criterion = filter_criterion
        self._filter_criterion_str = str(filter_criterion).replace(" ", "")
        self._metric_name = (
            f"fid_{self._embedding_syn.column_name}_{self._filter_criterion_str}"
            if filter_criterion
            else f"fid_{self._embedding_syn.column_name}"
        )

        self._priv_data = self._embedding_priv.compute_embedding(self._priv_data)
        priv_embedding = np.stack(self._priv_data.data_frame[self._embedding_priv.column_name].values, axis=0).astype(
            np.float32
        )
    ...
    def __call__(self, syn_data):
        ...
        """
        execution_logger.info(f"Computing FID ({self._embedding_syn.column_name}, {self._filter_criterion_str})")
        ...
        syn_data = self._embedding_syn.compute_embedding(syn_data)
        syn_embedding = np.stack(syn_data.data_frame[self._embedding_syn.column_name].values, axis=0).astype(np.float32)
        ...
        execution_logger.info(f"Finished computing FID ({self._embedding_syn.column_name}, {self._filter_criterion_str})")
        return [metric_item]
